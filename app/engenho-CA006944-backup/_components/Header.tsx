...
export default function Header() {
  ...
  const isOnLanding = pathname === "/engenho-CA006944";
  ...
  const navHref = (anchor: string) => (isOnLanding ? `#${anchor}` : `/engenho-CA006944#${anchor}`);

  const handleNavClick = (anchor: string) => (e: React.MouseEvent) => {
    if (!isOnLanding) {
      e.preventDefault();
      router.push(`/engenho-CA006944#${anchor}`);
    }
  ...
  const handleCtaClick = () => {
    ...
    if (isOnLanding) {
      ...
      router.push("/engenho-CA006944?openLeadForm=1");
    }
  };
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <Link href="/engenho-CA006944" className="flex items-center z-50 relative" aria-label="OPA Imóveis - Início">
            <img
