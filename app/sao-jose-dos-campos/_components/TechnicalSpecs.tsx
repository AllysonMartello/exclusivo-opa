...
const specIcons = [
  <Building2 size={24} />,    // Empreendimento
  <Home size={24} />,         // Edifício / Unidade
  <LayoutGrid size={24} />,   // Tipologia
  <Maximize size={24} />,     // Área privativa
  <Maximize2 size={24} />,    // Área total
  <Car size={24} />,          // Vagas
  <Bath size={24} />,         // Banheiros
  <Hammer size={24} />,       // Estrutura
  <ArrowUpDown size={24} />,  // Elevadores
  <PartyPopper size={24} />,  // Lazer
  <Sparkles size={24} />,     // Estado
  <Coins size={24} />,        // Condomínio
  <Receipt size={24} />,      // IPTU
  <MapPin size={24} />,       // Endereço
  <Landmark size={24} />,     // Financiamento
  <Wallet size={24} />,       // FGTS
  <Tag size={24} />,          // Valor (price card)
];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 sm:gap-x-8 gap-y-8 md:gap-y-12 mb-12 md:mb-20">
          {specs.filter((_, i) => i !== priceIndex).map((spec, index) => (
            <motion.div 
              ...
              className="flex flex-col items-start p-4 sm:p-6 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors duration-300"
            >
        ...
        <div className="flex justify-center">
          {[specs[priceIndex]].map((spec) => (
            <motion.div 
              ...
              className="flex flex-col items-center text-center p-8 sm:p-10 md:p-14 rounded-[2rem] md:rounded-[3rem] bg-white text-primary-1 shadow-[0_32px_64px_-15px_rgba(0,0,0,0.3)] max-w-lg w-full"
            >
