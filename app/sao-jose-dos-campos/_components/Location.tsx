...
const MAP_URL =
  "https://www.google.com/maps/place/Jardim+Sat%C3%A9lite,+S%C3%A3o+Jos%C3%A9+dos+Campos+-+SP/@-23.2406,-45.8902,15z";
...
export default function Location() {
  ...
  return (
    <section id="localizacao" className="py-16 md:py-24 lg:py-32 bg-bg-main relative overflow-hidden">
      ...
      <div className="hidden md:block absolute bottom-0 left-0 w-[600px] h-[600px] bg-primary-1/5 rounded-full blur-3xl translate-y-1/3 -translate-x-1/3 pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-12 lg:gap-20 items-center">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-primary-2 uppercase tracking-widest text-xs font-bold">
                {t.location.eyebrow}
              </span>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm p-6 md:p-8 rounded-[2rem] shadow-lg shadow-black/5">
              ...
              >
                {t.location.items.map((item, index) => {
                  const Icon = itemIcons[index] ?? ShoppingBag;
                  const imageSrc = itemImages[index];
                  const isExpanded = expandedIndex === index;
                  const hasAccordion = (item.description?.length ?? 0) > 0 || Boolean(imageSrc);

                  return (
                    <motion.li
                      key={index}
                      variants={itemVariants}
                      className={`group flex flex-col border-b border-border-main/15 pb-4 last:border-0 last:pb-0 transition-colors ${hasAccordion ? "cursor-pointer" : ""}`}
                      onClick={() => hasAccordion && toggleAccordion(index)}
                    >
                      <div className="flex items-center justify-between w-full gap-3">
                        <div className="flex items-center gap-3 md:gap-4 min-w-0">
                          <div className="w-10 h-10 rounded-full bg-primary-1/5 flex items-center justify-center text-primary-2 group-hover:bg-primary-2 group-hover:text-white transition-all duration-300 shadow-sm shrink-0">
                            <Icon size={18} />
                          </div>
                          <span className="text-text-main font-medium group-hover:text-primary-1 transition-colors text-sm md:text-base">
                            {item.label}
                          </span>
                        </div>
                        <span className="text-primary-2 font-bold bg-primary-1/5 px-3 md:px-4 py-1.5 rounded-full text-xs md:text-sm group-hover:bg-primary-2 group-hover:text-white transition-all duration-300 whitespace-nowrap">
                          {item.time}
                        </span>
                      </div>
          ...
          <motion.div
            ...
            className="lg:col-span-7 relative aspect-[4/5] sm:aspect-[4/5] lg:aspect-auto lg:h-[800px] rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl bg-bg-alt"
          >
            <a
              href={MAP_URL}
              target="_blank"
            ...
            <motion.div
              ...
              className="absolute top-4 left-4 md:top-8 md:left-8 bg-white/90 backdrop-blur-md p-3 md:p-4 rounded-2xl shadow-xl flex items-center gap-3 md:gap-4 z-20 max-w-[calc(100%-2rem)]"
            >
              ...
              <div className="min-w-0">
                <p className="text-[10px] md:text-xs font-bold text-primary-2 uppercase tracking-wider mb-0.5">
                  {t.location.coordinatesLabel}
                </p>
                <p className="text-xs md:text-sm font-mono text-primary-1 font-semibold whitespace-nowrap">
                  23°14'26"S 45°53'25"W
                </p>
              </div>
            </motion.div>
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
              <div className="relative">
                <div className="absolute -inset-6 bg-primary-2/20 rounded-full animate-ping" style={{ animationDuration: "3s" }} />
                <div className="absolute -inset-3 bg-primary-2/40 rounded-full animate-pulse" />
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-bg-main/90 via-transparent to-bg-main/30 pointer-events-none" />
          </motion.div>
        </div>
      </div>
    </section>
