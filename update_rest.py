...
content = re.sub(r'  experience: \{[\s\S]*?\}(?=,\n  forWho:)', new_experience, content)

# Update For Who Section
new_forwho = """  forWho: {
    eyebrow: "Para quem",
...
content = re.sub(r'  forWho: \{[\s\S]*?\}(?=,\n  virtualTour:)', new_forwho, content)

# Update Technical Specs
new_specs = """  technicalSpecs: {
    title: "Ficha Técnica",
    specs: [
...
content = re.sub(r'  technicalSpecs: \{[\s\S]*?\}(?=,\n  theOPA:)', new_specs, content)
...
content = re.sub(r'  theOPA: \{[\s\S]*?\}(?=,\n  marcosView:)', new_theopa, content)

# Update marcosView
new_marcos = """  marcosView: {
    eyebrow: "Atendimento",
    name: "Marco Henrique",
...
content = re.sub(r'  marcosView: \{[\s\S]*?\}(?=,\n  finalCTA:)', new_marcos, content)

# Update finalCTA
new_finalcta = """  finalCTA: {
...
content = re.sub(r'  finalCTA: \{[\s\S]*?\}(?=,\n  footer:)', new_finalcta, content)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print("Updated the rest of translations.ts")
