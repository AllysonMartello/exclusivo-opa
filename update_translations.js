...
content = content.replace(/  hero: \{[\s\S]*?\},/, new_pt_hero);

const new_pt_virtualTour = `  virtualTour: {
...
content = content.replace(/  virtualTour: \{[\s\S]*?\},/, new_pt_virtualTour);

const new_pt_location = `  location: {
    eyebrow: "Localização",
...
content = content.replace(/  theHouse: \{[\s\S]*?\}(?=,\n  experience:)/, new_pt_theHouse);

const new_pt_experience = `  experience: {
...
content = content.replace(/  experience: \{[\s\S]*?\}(?=,\n  forWho:)/, new_pt_experience);

const new_pt_forWho = `  forWho: {
    eyebrow: "Para quem",
...
content = content.replace(/  forWho: \{[\s\S]*?\}(?=,\n  virtualTour:)/, new_pt_forWho);

const new_pt_tech = `  technicalSpecs: {
    title: "Ficha Técnica",
    specs: [
...
content = content.replace(/  theOPA: \{[\s\S]*?\}(?=,\n  marcosView:)/, new_pt_opa);

const new_pt_marcos = `  marcosView: {
    eyebrow: "Atendimento",
    name: "Marco Henrique",
...
content = content.replace(/  marcosView: \{[\s\S]*?\}(?=,\n  finalCTA:)/, new_pt_marcos);

const new_pt_final = `  finalCTA: {
